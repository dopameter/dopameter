...
class ConstituencyFeatures:
    ...
    * or in detail:
    ...
    def feat_corpus(self, corpus):
        ...
        if corpus.syntax_const['tree_heights']:
            data['features']['AvgMaxDepth'] = np.mean(corpus.syntax_const['tree_heights'])
        else:
            data['features']['AvgMaxDepth'] = 0

        if corpus.syntax_const['doc_out_degrees']:
            data['features']['AvgFan'] = np.mean(corpus.syntax_const['doc_out_degrees'])
        else:
            data['features']['AvgFan'] = 0

        if corpus.syntax_const['non_terminals_wo_leaves']:
            data['features']['AvgNonTerminales_sent'] = np.mean(corpus.syntax_const['non_terminals_wo_leaves'])
        else:
            data['features']['AvgNonTerminales_sent'] = 0

        if corpus.syntax_const['doc_const'] and corpus.sentences_cnt > 0:
            data['features']['AvgConstituents_sent'] = len(corpus.syntax_const['doc_const']) / corpus.sentences_cnt
        else:
            data['features']['AvgConstituents_sent'] = 0

        if corpus.syntax_const['t_units'] and corpus.sentences_cnt > 0:
            data['features']['AvgTunits_sent'] = len(corpus.syntax_const['t_units']) / corpus.sentences_cnt
        else:
            data['features']['AvgTunits_sent'] = 0

        len_t_units = len([i for i in corpus.syntax_const['t_units'] if i > 0])
        if len_t_units > 0:
            data['features']['AvgLenTunits'] = sum(corpus.syntax_const['t_units']) / len_t_units
        else:
            data['features']['AvgLenTunits'] = 0

        if corpus.syntax_const['out_degree_centralization']:
            data['features']['AvgOutdegreeCentralization'] = np.mean(corpus.syntax_const['out_degree_centralization'])
        else:
            data['features']['AvgOutdegreeCentralization'] = 0

        if corpus.syntax_const['closeness_centrality']:
            data['features']['AvgClosenessCentralization'] = np.mean(corpus.syntax_const['closeness_centrality'])
        else:
