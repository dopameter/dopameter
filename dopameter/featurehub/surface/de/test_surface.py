...
class TestSurface(unittest.TestCase):
    ...
    def test_rb(self):
        ...
        nlp = PreProcessingPipline(config = {'features' : {'surface': 'default'}}).create_nlp('de')
        ...
        self.assertEqual(surface_metrics['counts']['toks_min_three_syllables'], 48)
        self.assertEqual(surface_metrics['counts']['toks_larger_six_letters'], 84)
        self.assertEqual(surface_metrics['counts']['toks_one_syllable'], 169)
        self.assertEqual(surface_metrics['counts']['syllables'], 500)
        self.assertEqual(surface_metrics['counts']['letter_tokens'], 302)
        self.assertEqual(surface_metrics['counts']['no_digit_tokens'], 308)

        self.assertEqual( round(surface_metrics['features']['avg_token_len_chars'],2), 4.64)
        self.assertEqual( round(surface_metrics['features']['avg_sent_len_tokens'],2), 12.41)
        self.assertEqual( round(surface_metrics['features']['avg_sent_len_chars'],2), 67.28)
        self.assertEqual( round(surface_metrics['features']['flesch_kincaid_grade_level'],2), 7.71)
        self.assertEqual( round(surface_metrics['features']['smog'],2),  0)
        self.assertEqual( round(surface_metrics['features']['coleman_liau'],2), 11.94)
        self.assertEqual( round(surface_metrics['features']['ari'],2), 8.62)
        self.assertEqual( round(surface_metrics['features']['forcast'],2), 14.37)
        self.assertEqual( round(surface_metrics['features']['gunning_fog'],2),  10.48)
        self.assertEqual( round(surface_metrics['features']['wiener_sachtextformel_1'],2), 1.25)
        self.assertEqual( round(surface_metrics['features']['wiener_sachtextformel_2'],2),  -0.63)
        self.assertEqual( round(surface_metrics['features']['wiener_sachtextformel_3'],2), 1.29)
        self.assertEqual( round(surface_metrics['features']['wiener_sachtextformel_4'],2),1.17)
        self.assertEqual( round(surface_metrics['features']['flesch_reading_ease'],2), 84.42)
        self.assertEqual( round(surface_metrics['features']['heylighen_formality'],2), 59.85)
