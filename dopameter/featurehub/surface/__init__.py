    ...
    def feat_corpus(self, corpus):
        ...
        if 'forcast' in self.features:
            data['features']['forcast'] = self.forcast(
                corpus.resources.surface['cnt_words'],
                corpus.resources.surface['corpus_segments']
            )
...
def update_surface(surface, data):

    if 'token_len_chars' in data.keys():
        surface['token_len_chars'] += data['token_len_chars']
    if 'sent_len_tokens' in data.keys():
    ...
    if 'cnt_words' in data.keys():
        surface['cnt_words'] += data['cnt_words']
    if 'cnt_poly_syllables' in data.keys():
