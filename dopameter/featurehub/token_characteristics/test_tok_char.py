...
class TestTokenCharacteristics(unittest.TestCase):
    ...
    def test_pos(self):
        ...
        tc = tokchar.feat_doc(doc=doc)

        self.assertEqual(tc['counts']['is_alpha'], 109)
        self.assertEqual(tc['counts']['is_ascii'], 124)
        self.assertEqual(tc['counts']['is_digit'], 0)
        self.assertEqual(tc['counts']['is_lower'], 86)
        self.assertEqual(tc['counts']['is_upper'], 0)
        self.assertEqual(tc['counts']['is_title'], 23)
        self.assertEqual(tc['counts']['is_punct'], 18)
        self.assertEqual(tc['counts']['is_left_punct'], 0)
        self.assertEqual(tc['counts']['is_right_punct'], 0)
        self.assertEqual(tc['counts']['is_space'], 0)
        self.assertEqual(tc['counts']['is_bracket'], 0)
        self.assertEqual(tc['counts']['is_quote'], 0)
        self.assertEqual(tc['counts']['is_currency'], 0)
        self.assertEqual(tc['counts']['like_url'], 0)
        self.assertEqual(tc['counts']['like_num'], 0)
        self.assertEqual(tc['counts']['like_email'], 0)
        self.assertEqual(tc['counts']['is_stop'], 75)
