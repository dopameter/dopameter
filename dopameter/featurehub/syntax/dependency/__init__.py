...
class DependencyFeatures:
    ...
    * or in detail:
    ...
    def feat_corpus(self, corpus):
        ...
        if corpus.resources.syntax_dep['children_per_node']:
            data['features']['AvgFan'] = np.mean(corpus.resources.syntax_dep['children_per_node'])
            data['features']['MaxFan'] = np.max(corpus.resources.syntax_dep['children_per_node'])
        else:
            data['features']['AvgFan'] = 0
            data['features']['MaxFan'] = 0

        if corpus.resources.syntax_dep['max_depth']:
            data['features']['AvgMaxDepth'] = np.mean(corpus.resources.syntax_dep['max_depth'])
        else:
            data['features']['AvgMaxDepth'] = 0

        if not corpus.resources.syntax_dep['dep_distance']:
            data['features']['AvgDepDist'] = 0
            data['features']['MaxDepDist'] = 0
        else:
            data['features']['AvgDepDist'] = np.mean(corpus.resources.syntax_dep['dep_distance'])
            data['features']['MaxDepDist'] = np.max(corpus.resources.syntax_dep['dep_distance'])

        if corpus.resources.syntax_dep['out_degree_centralization']:
            data['features']['AvgOutdegreeCentralization'] = np.mean(corpus.resources.syntax_dep['out_degree_centralization'])
        else:
            data['features']['AvgOutdegreeCentralization'] = 0

        if corpus.resources.syntax_dep['closeness_centrality']:
            data['features']['AvgClosenessCentralization'] = np.mean(corpus.resources.syntax_dep['closeness_centrality'])
        else:
            data['features']['AvgClosenessCentralization'] = 0

        return data

def init_syntax_dependency_metrics():
    return {
        'children_per_node': [],
        'max_depth': [],
        'dep_distance': [],
...
def update_syntax_dependency_metrics(syntax_dep, data):

    for key in data:
        syntax_dep[key] += data[key]
