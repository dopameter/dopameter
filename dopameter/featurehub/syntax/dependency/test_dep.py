...
class TestDependency(unittest.TestCase):
    ...
    def test_rb(self):
        ...
        doc = nlp(text)
        sd_metrics = syntax_dependency.feat_doc(doc=doc)

        self.assertEqual( round(sd_metrics['syntax_dependency_metrics']['features']['AvgFan'],2), 2.18)
        self.assertEqual( round(sd_metrics['syntax_dependency_metrics']['features']['MaxFan'],2), 11)
        self.assertEqual( round(sd_metrics['syntax_dependency_metrics']['features']['AvgMaxDepth'],2), 4.59)
        self.assertEqual( round(sd_metrics['syntax_dependency_metrics']['features']['AvgDepDist'],2), 2.51)
        self.assertEqual( round(sd_metrics['syntax_dependency_metrics']['features']['MaxDepDist'],2), 6.53)
        self.assertEqual( round(sd_metrics['syntax_dependency_metrics']['features']['AvgOutdegreeCentralization'],2), 0.46)
        self.assertEqual( round(sd_metrics['syntax_dependency_metrics']['features']['AvgClosenessCentralization'],2), 0.37)

        self.assertEqual( sd_metrics['syntax_dependency_tree']['counts']['ROOT'], 29)
        self.assertEqual( sd_metrics['syntax_dependency_tree']['counts']['punct'], 51)
        self.assertEqual( sd_metrics['syntax_dependency_tree']['counts']['nk'], 60)
        self.assertEqual( sd_metrics['syntax_dependency_tree']['counts']['svp'], 1)
